...
namespace Lenia {
	Animal::Animal() : m_taxonomy({}), m_r(0), m_dt(0.f), m_beta(nullptr),
		m_b(0), m_mu(0.f), m_sigma(0.f), m_kn(KernelCore::QUAD4), m_gn(GrowthFunction::QUAD4), m_w(0), m_h(0), m_rle(""), m_dx2(0.f), m_kernelBuffer() {}
	...
	Animal::Animal(const Taxonomy taxonomy, const u32 r, const f32 dt, const f32* beta, const u8 b, const f32 mu, const f32 sigma,
		...
		m_taxonomy(taxonomy), m_r(r), m_dt(dt), m_beta(beta), m_b(b), m_mu(mu), m_sigma(sigma), m_kn(kn), m_gn(gn), m_rle(rle), m_w(0), m_h(0), m_dx2(1.f), m_kernelBuffer() {}
	...
	void Animal::Bind() {
		m_kernelBuffer = Buffer<f32>(m_r * m_r, BufferBinding::KERNEL);
		ComputeKernel();
	}
	...
	std::unique_ptr<f32[]> Animal::GetCells() noexcept {
		char* str = (char*)this->m_rle.c_str();
	...
	f32 Animal::ApplyGrowthFunction(const f32 n) const {
		...
		case GrowthFunction::QUAD4:
			return std::powf(std::fmax(0.f, 1.f - std::powf(n - m_mu, 2.f) / (9.f * m_sigma * m_sigma)), 4.f) * 2.f - 1.f;
		case GrowthFunction::GAUSS:
			return std::expf(-std::powf(n - m_mu, 2.f) / (2.f * m_sigma * m_sigma)) * 2.f - 1.f;
	...
	void Animal::ComputeKernel() const {

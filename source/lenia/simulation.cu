...
{
    f32 *buffer_gpu;
    std::size_t size = m_readBuffer.m_data.size();
    cudaMalloc(&buffer_gpu, size * sizeof(f32));
    cudaMemcpy(buffer_gpu, m_readBuffer.m_data.data(), size * sizeof(f32), cudaMemcpyHostToDevice);
    thrust::transform(
        thrust::device,
        buffer_gpu,
        buffer_gpu + size,
    ...
    for (std::size_t i = 0; i < batches; ++i)
    {
        ...
        for (u8 i = 0; i < 4; ++i)
        {
            ...
            quadrant_infos[i] = thrust::transform_reduce(
                ...
                FieldTupleToLayerInfoFunctor{m_w / 2},
                Lenia::LayerInfo{Lenia::BoundingBox{INT_MAX, INT_MAX, INT_MIN, INT_MIN}, glm::vec2{}, 0.f},
                _1 + _2);
            quadrant_infos[i].m_centerOfMass /= quadrant_infos[i].m_mass;
        }
